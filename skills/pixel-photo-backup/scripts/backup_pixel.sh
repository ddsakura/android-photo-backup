...
for PHONE_DIR in "${PHONE_DIRS[@]}"; do
  ...
  # 取得手機上的檔案清單
  if FILE_LIST=$(adb -s "$ADB_SERIAL" shell find "$PHONE_DIR" -type f 2>/dev/null); then
    FIND_STATUS=0
  else
    FIND_STATUS=$?
  fi
  ...
  while IFS= read -r PHONE_FILE; do
    ...
    if [ -f "$LOCAL_FILE" ]; then
      # 已存在：比對檔案大小，大小一樣就跳過
      if ! PHONE_SIZE=$(adb -s "$ADB_SERIAL" shell stat -c%s "$PHONE_FILE" 2>/dev/null | tr -d '\r'); then
        PHONE_SIZE=""
      fi
      if ! LOCAL_SIZE=$(stat -f%z "$LOCAL_FILE" 2>/dev/null); then
        LOCAL_SIZE=""
      fi
    ...
    # 拉取檔案
    if adb -s "$ADB_SERIAL" pull "$PHONE_FILE" "$LOCAL_FILE" &>/dev/null; then
      NEW=$((NEW + 1))
    else
